...
| Tests/builds/linters | `ctx_execute` with `output_mode: "diagnostics"` | `npm test`, `pytest`, `tsc`, `mvn test` |
...
- Running `npm test` via Bash → full test output in context. Use `ctx_execute(language: "shell", code: "npm test", output_mode: "diagnostics")` so full stdout/stderr is indexed and warnings/errors are summarized with context.

...
EXAMPLE: ctx_execute(language: "javascript", code: "const out = require('child_process').execSync('gh issue list --json number,title --limit 100', {encoding:'utf8'}); const hooks = JSON.parse(out).filter(i => /hook|routing/i.test(i.title)); console.log(\`\${hooks.length} hook-related issues\`)")`,
    inputSchema: z.object({
      ...
      output_mode: z
        ...
        .default("auto")
        .describe(
          "Controls what enters conversation context. auto preserves legacy behavior. " +
          "diagnostics indexes full stdout/stderr and returns error/warning/failure windows with nearby context. " +
          "compact returns a small diagnostic summary. full returns stdout+stderr verbatim. " +
          "index stores stdout+stderr and returns only a searchable source pointer.",
        ),
    }),
  },
  async ({ language, code, timeout, background, cwd, intent, output_mode }) => {
...
__cm_main().catch(e=>{console.error(e);process.exitCode=1});${background ? '\nsetInterval(()=>{},2147483647);' : ''}
      ...
      const outputMode = output_mode as OutputMode;
      if (outputMode !== "auto") {
        const rawCombined = combineExecOutput(result);
        const combined = rawCombined || "(no output)";
        const classified = result.exitCode !== 0
          ? classifyNonZeroExit({
            language, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr,
          })
          : { isError: false };
        const isError = result.timedOut || classified.isError;
        const timeoutNote = result.timedOut
          ? `\n\n_(timed out after ${effTimeout}ms${result.backgrounded ? " — process backgrounded and still running" : ""})_`
          : "";

        if (outputMode === "full") {
          return trackResponse("ctx_execute", {
            content: [
              { type: "text" as const, text: `${echo}${combined}${timeoutNote}` },
            ],
            isError,
          });
        }
        ...
        if (outputMode === "index") {
          if (!rawCombined) {
            return trackResponse("ctx_execute", {
              content: [
                { type: "text" as const, text: `${echo}No output to index.${timeoutNote}` },
              ],
              isError,
            });
          }
          const label = indexExecutionOutput(rawCombined, `execute:${language}`);
          return trackResponse("ctx_execute", {
            content: [
              { type: "text" as const, text: `${echo}Indexed output as source: "${label}"\nUse ctx_search(queries: ["..."], source: "${label}") to inspect it.${timeoutNote}` },
            ],
            isError,
          });
        }

        const shouldIndexDiagnostics = outputMode === "diagnostics" && rawCombined.length > 0;
        const sourceLabel = shouldIndexDiagnostics
          ? indexExecutionOutput(rawCombined, result.exitCode !== 0 ? `execute:${language}:error` : `execute:${language}:diagnostics`)
          : undefined;
        const summary = summarizeDiagnostics(
          ...
          outputMode === "compact"
            ? { contextLines: 1, maxWindows: 8, maxBytes: 6_000 }
            : undefined,
        );
        return trackResponse("ctx_execute", {
          content: [
            { type: "text" as const, text: `${echo}${summary}${timeoutNote}` },
          ],
          isError,
        });
      }

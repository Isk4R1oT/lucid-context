  ...
  if (canonical === "Bash") {
    ...
    if (/(^|\s|&&|\||\;)(\.\/gradlew|gradlew|gradle|\.\/mvnw|mvnw|mvn|\.\/sbt|sbt)(\s|$)/i.test(stripped)) {
      ...
      return mcpRedirect({
        action: "modify",
        updatedInput: {
          command: `echo "context-mode: Build tool redirected. Call ${t("ctx_execute")}(language: \\"shell\\", code: \\"${safeCmd}\\", output_mode: \\"diagnostics\\") to run the build, index the full stdout/stderr, and return diagnostic windows around errors, warnings, failures, and test summaries. Do not use tail-only filters for builds/tests — important diagnostics can appear before the final lines."`,
        },
      }, mcpToolsAvailable);
    }

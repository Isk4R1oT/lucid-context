...
` : ''}
  <tool_selection_hierarchy>
    ...
    3. PROCESSING: ${t("ctx_execute")}(language, code) | ${t("ctx_execute_file")}(path, language, code)
       ...
       - For tests, builds, compilers, and linters, pass output_mode: "diagnostics" so full stdout/stderr is indexed while only error/warning/failure windows enter the conversation.
  </tool_selection_hierarchy>
...
export function createBashGuidance(t) {
  return '<context_guidance>\n  <tip>\n    When you intend to PROCESS the output (filter, count, parse, aggregate), use ' + t("ctx_batch_execute") + '(commands, queries) for multiple commands or ' + t("ctx_execute") + '(language: "javascript", code: "...") for one — the raw output stays in the sandbox and only what you print enters your conversation. For tests/builds/compilers/linters, use ' + t("ctx_execute") + '(language: "shell", code: "...", output_mode: "diagnostics") so warnings and errors are preserved even when they are not in the tail. Shell stays the right surface when you intend to OBSERVE a short fixed output or when you are mutating state (git, mkdir, rm, mv, navigation); if you use ' + t("ctx_execute") + '(language: "shell"), write syntax for the host shell.\n  </tip>\n</context_guidance>';
}
